...
class EvaluationService:
    ...
    Deliberately has no LangGraph, no RAG, no Redis dependency: a
    one-shot structured LLM call over a finished transcript needs none of
    the interview workflow's adaptive orchestration.
    """

    def __init__(self, repository: InterviewRepository, llm_provider: LLMProvider):
    ...
    async def get_or_create_evaluation(self, session_id: uuid.UUID) -> Evaluation:
        ...
        invoking the LLM again. Only a genuinely new evaluation reaches
        the LLM call and the durable-mutation transaction below."""
        ...
        try:
            # A concurrent request may have already persisted its own
            # evaluation while this one was waiting on the LLM — recheck
            # before inserting so the common (non-racing) case never even
            # reaches the unique-constraint violation below.
            existing = self.repository.get_evaluation(session_id)
            if existing is not None:
                return existing
